...
def _install_model_route_import_stubs(monkeypatch):
    ...
    log_safety_mod.redact_url = lambda url, maxlen=2048: url
...
def _install_core_middleware_stub(monkeypatch):
    ...
    core_mod.__path__ = []
    middleware_mod = types.ModuleType("core.middleware")
    middleware_mod.INTERNAL_TOOL_HEADER = "X-Ulises-Internal-Token"
...
async def test_disabled_qualified_email_tool_blocks_bare_alias(monkeypatch):
    ...
    for tool, disabled in (
        ("mcp__email__list_emails", {"mcp__email__list_emails"}),
        ("mcp__email__download_attachment", {"mcp__email__download_attachment"}),
        ("mcp__email__delete_email", {"mcp__email__delete_email"}),
    ):
        desc, result = await execute_tool_block(
            SimpleNamespace(tool_type=tool, content="{}"),
            owner="admin-user",
...
async def test_tool_policy_qualified_email_block_covers_bare_alias(monkeypatch):
    ...
    desc, result = await execute_tool_block(
        SimpleNamespace(tool_type="mcp__email__send_email", content="{}"),
        owner="admin-user",
        tool_policy=policy,
    )
    assert desc == "mcp__email__send_email: BLOCKED"
    assert result["exit_code"] == 1
...
async def test_disable_tool_email_covers_full_builtin_set(monkeypatch):
    ...
    mcp_email_tools = {
        "mcp__email__list_emails", "mcp__email__read_email", "mcp__email__send_email",
    }
    for tool_name in mcp_email_tools:
...
async def test_bare_email_dispatch_rejects_invalid_json_body(monkeypatch):
    ...
    for bad_body in ('{account: "work"}', "account: work"):
        ...
        assert "Unknown tool" in result["error"], bad_body


@pytest.mark.asyncio
async def test_legacy_mcp_tools_decode_inline_json_args(monkeypatch):
    """Line-based arg builders use the first line as the primary key value."""
    import src.tool_execution as tool_execution
    from src.tool_execution import _build_mcp_args

    cases = {
        "web_search": ('{"query": "odysseus pr 3681"}', {"query": '{"query": "odysseus pr 3681"}'}),
        "web_fetch": ('{"url": "https://example.com"}', {"url": '{"url": "https://example.com"}'}),
        "read_file": ('{"path": "/tmp/x.txt"}', {"path": '{"path": "/tmp/x.txt"}'}),
        "write_file": ('{"path": "/tmp/x", "content": "hi"}', {"path": '{"path": "/tmp/x", "content": "hi"}', "content": ""}),
        "generate_image": ('{"prompt": "a cat"}', {"prompt": '{"prompt": "a cat"}'}),
    }
    ...
    # write_file uses _parse_write_file which splits on first newline.
    assert _build_mcp_args("write_file", '{"config": "value"}') == {
        "path": '{"config": "value"}', "content": "",
    }
    # Two-line write_file correctly splits path and content.
    assert _build_mcp_args("write_file", "/tmp/x\nhi there") == {
        "path": "/tmp/x", "content": "hi there",
    }


def test_mcp_tool_map_entries_have_parsers():
    """Every _MCP_TOOL_MAP entry must have a corresponding parser in _MCP_ARG_PARSERS."""
    from src.tool_execution import _MCP_TOOL_MAP, _MCP_ARG_PARSERS

    missing = set(_MCP_TOOL_MAP) - set(_MCP_ARG_PARSERS)
    assert not missing, f"MCP tools missing arg parsers: {sorted(missing)}"
...
async def test_write_file_inline_json_args(monkeypatch):
    ...
    blocks = parse_tool_blocks('```write_file\n/tmp/wf.txt\nhi\n```')
    for b in blocks:
...
async def test_plan_mode_blocks_mutating_email_aliases_without_mcp_inventory(monkeypatch):
    ...
    for tool_name in ("mcp__email__draft_email", "mcp__email__draft_email_reply",
                      "mcp__email__ai_draft_email_reply", "mcp__email__download_attachment",
        ...
        if tool_name in denied:
            ...
            assert result["exit_code"] == 0, tool_name

    desc, result = await execute_tool_block(
        SimpleNamespace(tool_type="mcp__email__search_emails", content='{"query": "x"}'),
        owner="admin-user",
        disabled_tools=denied,
    )
    assert result["exit_code"] == 0
...
async def test_bare_email_dispatch_empty_content_calls_with_empty_args(monkeypatch):
    ...
    desc, result = await execute_tool_block(
        SimpleNamespace(tool_type="mcp__email__list_email_accounts", content=""),
        owner="admin-user",
    )
    assert result["exit_code"] == 0
...
async def test_email_mcp_non_object_args_fail_before_dispatch(monkeypatch):
    ...
    assert result["exit_code"] == 0
    assert fake.calls == [("mcp__email__list_emails", {})]
...
async def test_email_mcp_dispatch_includes_hidden_owner(monkeypatch):
    ...
    assert result["exit_code"] == 0
    assert fake.calls == [
        ("mcp__email__list_emails", {"folder": "INBOX"}),
    ]
