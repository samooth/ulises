...
Ulises keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/ulises-backup` tool
...
```bash
# Create a snapshot → backups/ulises-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/ulises-backup snapshot

# List existing snapshots (most recent first)
./scripts/ulises-backup list

# Check a tarball's integrity without extracting it
./scripts/ulises-backup verify backups/ulises-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/ulises-backup restore backups/ulises-backup-20260101-120000.tar.gz --yes
```
...
./scripts/ulises-backup snapshot --out /mnt/nas/ulises-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/ulises-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/ulises && ./scripts/ulises-backup snapshot --out "/mnt/nas/ulises-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `ulises-backup` therefore does not capture the Docker
