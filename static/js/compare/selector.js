...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    const _selKey = 'ulises-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('ulises-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('ulises-compare-synth-' + state._compareMode);
